...
class ${prefix.charAt(0).toUpperCase()+prefix.substr(1)}Controller extends Controller {
    /**@apiIgnore
     * @api {POST} ${basePrefix+prefix} ${apiNum}.1创建${tableDesc}
    ...
    /**@apiIgnore
     * @api {GET} ${basePrefix+prefix} ${apiNum}.2查找${tableDesc}
    ...
    /**@apiIgnore
     * @api {PUT} ${basePrefix+prefix} ${apiNum}.3更新${tableDesc}
    ...
    /**@apiIgnore
     * @api {DELETE} ${basePrefix+prefix} ${apiNum}.4删除${tableDesc}
...
class ${prefix.charAt(0).toUpperCase()+prefix.substr(1)}Service extends Service {
    ...
    async del(id){
        return await this.doQueryError(
            \`delete from ${table} where id=\${this.ctx.params.id}\`
        )
    }
}
